...
try:
    ...
    _CANVAS_PROTOBUF_AVAILABLE = True
    _CANVAS_PROTOBUF_ERROR = None
except Exception as exc:
    EntityCanvazRequest = None
    EntityCanvazResponse = None
    _CANVAS_PROTOBUF_AVAILABLE = False
    _CANVAS_PROTOBUF_ERROR = exc
    log.warning("Spotify Canvas disabled: protobuf runtime unavailable (%s)", exc)
...
def _fetch_canvas(token, track_uri):
    ...
    if not _CANVAS_PROTOBUF_AVAILABLE:
        return None
...
class SpotifyCanvasClient:
    ...
    @property
    def configured(self):
        return bool(self._sp_dc) and _CANVAS_PROTOBUF_AVAILABLE
